...
export async function getStaticProps({ params }: any) {
  ...
  const res = await fetch(
    `https://admin-wp-back.canalapps.com/wp-json/wp/v2/posts?categories=${category[0].id}&page=${params.page}&per_page=9`
  );
...
export async function getStaticPaths() {
  ...
  categories.map(async (category: CategoryProps) => {
    const catRes = await fetch(
      `https://admin-wp-back.canalapps.com/wp-json/wp/v2/posts?categories=${category.id}&per_page=9`
    );
